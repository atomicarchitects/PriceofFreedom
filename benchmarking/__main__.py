...
from src.tensor_products.functional import (
    clebsch_gordan_tensor_product_dense,
    clebsch_gordan_tensor_product_sparse,
    gaunt_tensor_product_s2grid,
    gaunt_tensor_product_fourier_2D,
)
...
flags.DEFINE_enum("irreps_type", "MIMO", ["MIMO", "SIMO", "SISO"], "Input/Output Irreps")
flags.DEFINE_enum(
    "tensor_product_type",
    "CGTP-dense",
...
flags.DEFINE_bool("ncu_flag", False, "Flag for Nsight Compute Benchmarking")
flags.DEFINE_string("walltime_file", "benchmarking/csv/walltime.csv", "Path to Walltime file")
...
def tp_initializer(tp_type, output_irreps):
    if tp_type == "CGTP-dense":
        ...
        )  # Remember to try with fft as well"
    else:
...
def benchmark_per_lmax(lmax: int, tp_type: str, irreps_type: str, batch: int):
    ...
    TP = jax.jit(TP)
    print(
        f"irreps_type {FLAGS.irreps_type} tensor_product_type {FLAGS.tensor_product_type} lmax {FLAGS.lmax} batch {FLAGS.batch}"
    )
...
def main():
    benchmark_per_lmax(FLAGS.lmax, FLAGS.tensor_product_type, FLAGS.irreps_type, FLAGS.batch)
