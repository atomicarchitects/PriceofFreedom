...
def to_graph(data: pyG.data.Data, target_property_index: int) -> jraph.GraphsTuple:
    ...
    data = data.to_dict()
    data = {key: val.numpy() if isinstance(val, torch.Tensor) else val for key, val in data.items()}
...
class QM9Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, root: str, target_property: str, cutoff: float, add_self_edges: bool, splits: Dict[str, int], seed: int):
        self.cutoff = cutoff
        ...
        properties = {
            ...
            1: {'name': 'alpha', 'description': 'Isotropic polarizability', 'unit': 'a_0^3'},
            2: {'name': 'epsilon_HOMO', 'description': 'Highest occupied molecular orbital energy', 'unit': 'eV'},
            3: {'name': 'epsilon_LUMO', 'description': 'Lowest unoccupied molecular orbital energy', 'unit': 'eV'},
            4: {'name': 'Delta epsilon', 'description': 'Gap between epsilon_HOMO and epsilon_LUMO', 'unit': 'eV'},
            5: {'name': 'R^2', 'description': 'Electronic spatial extent', 'unit': 'a_0^2'},
            6: {'name': 'ZPVE', 'description': 'Zero point vibrational energy', 'unit': 'eV'},
            ...
            11: {'name': 'c_v', 'description': 'Heat capavity at 298.15K', 'unit': 'cal/(mol K)'},
            12: {'name': 'U_0_ATOM', 'description': 'Atomization energy at 0K', 'unit': 'eV'},
            13: {'name': 'U_ATOM', 'description': 'Atomization energy at 298.15K', 'unit': 'eV'},
            14: {'name': 'H_ATOM', 'description': 'Atomization enthalpy at 298.15K', 'unit': 'eV'},
            15: {'name': 'G_ATOM', 'description': 'Atomization free energy at 298.15K', 'unit': 'eV'},
        ...
        for key, value in properties.items():
            if value['name'] == target_property:
                ...
                logging.info(f"Target property {target_property}: {value['description']} ({value['unit']})")
                break
        if target_property_index is None:
            raise ValueError(f"Unknown target property {target_property}. Available properties are: {', '.join([value['name'] for value in properties.values()])}")
        self.target_property_index = target_property_index
        
        # Split the dataset.
        dataset = pyG.datasets.QM9(root=root, transform=None, pre_transform=utils.add_edges_transform(cutoff=cutoff, add_self_edges=add_self_edges))
        self.datasets = utils.split_dataset(dataset, splits, seed)

    def to_jraph_graphs(self, split: str, batch_size: int) -> Iterator[jraph.GraphsTuple]:
        ...
        while True:
            for data in self.datasets[split]:
                data_as_jraph = to_graph(data, target_property_index=self.target_property_index)
                ...
                if len(batch) == batch_size:
                    ...
                    if padding is None:
                        padding = dict(
                            n_node=int(2.0 * np.ceil(batch_jraph.n_node.sum() / 64) * 64),
                            n_edge=int(2.0 * np.ceil(batch_jraph.n_edge.sum() / 64) * 64),
